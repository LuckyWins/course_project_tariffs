...
class PreferencesDataSource {
  ...
  static Future<T?> read<T>(TypeStoreKey<T> typedStoreKey) async {
    await reload();
    return ((await prefs).get(typedStoreKey.key) as T?) ??
        typedStoreKey.defaultValue;
  }

  static Future<bool> contains(TypeStoreKey typedStoreKey) async {
    await reload();
    return (await prefs).containsKey(typedStoreKey.key);
  }
  ...
  static Future<void> write<T>(TypeStoreKey<T> typedStoreKey, T? value) async {
    ...
    switch (T) {
      ...
      case List:
        await (await prefs)
            .setStringList(typedStoreKey.key, value as List<String>);
        break;
    }
  }
}
...
class PrefsKeys {
  static final themeMode = TypeStoreKey<String>('themeMode');
  static final language = TypeStoreKey<String>('language', defaultValue: 'en');
}
...
class TypeStoreKey<T> {
  ...
  TypeStoreKey(this.key, {this.defaultValue});
